...
class MD_CA(nn.Module):
    r"""
    This code is mainly built on Adaptive Token Dictionary Cross-Attention.
    ...
    def forward(self, x, td, td_2, x_size):
        ...
        Args:
            ...
            td_2: token dicitionary with shape of (b, m, c) from Deep Dictionary Cross-Attention
...
class DHAB(nn.Module):
    r"""
    Deep Hierarchical Attention Block
    This code is mainly built on Adaptive Category-based Multihead Self-Attention.
    ...
    def __init__(self, dim, input_resolution, is_first,num_tokens=64, num_heads=4, category_size=128, qkv_bias=True,norm_layer=nn.LayerNorm,alpha = 0.2):
    ...
    def forward(self, qkv, sim,sim2, x_size):
        ...
        y= y + self.alpha * y2 + self.alpha * x_atd2
...
class HAB(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.ChannelAttentionBlock = ChannelAttentionBlock(dim)
        self.SpatialAttentionBlock = SpatialAttentionBlock(dim)
        self.FrequencyAttentionBlock = FrequencyAttentionBlock(dim)
    ...
    def forward(self, x, H, W):
        ...
        channel_map = self.ChannelAttentionBlock(attention_reshape)
        ...
        spatial_map = self.SpatialAttentionBlock(conv_x).permute(0, 2, 3, 1).contiguous().view(B, N, C)
        frequenece_map = self.FrequencyAttentionBlock(conv_x).permute(0, 2, 3, 1).contiguous().view(B, N, C)
        # S-I
...
class DDCA(nn.Module):
    r"""
    Deep Dictionary Cross-Attention,which is same as MD_CA.
