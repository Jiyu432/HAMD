...
class ATD_CA(nn.Module):
    ...
    def forward(self, x, td, x_size):
        ...
        # K: b, m, c
        k = self.wk(td)
        k_expanded = k.repeat(1, 32, 1)
        k_expanded = q - k_expanded
        # V: b, m, c
        v = self.wv(td)
        for i in range(0,3):

            attn = (F.normalize(k_expanded, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))
            if i!=9:
                attn=attn.mean(dim=-1, keepdim=True)

            k_expanded =attn-k_expanded.mean(dim=-1, keepdim=True)
            k_expanded = k_expanded.repeat(1, 1, 20)
            # Q @ K^T
...
class AC_MSA(nn.Module):
    ...
    def forward(self, qkv, sim,sim2, x_size):
        ...
        # 全局注意力计算
        if sim2 is not None:
            tk_id2 = torch.argmax(sim2, dim=-1, keepdim=False)
            # sort features by type
...
class ATDTransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_last:
            ...
            self.sigma2 = nn.Parameter(torch.zeros([16, 1]), requires_grad=True)
        ...
        self.ca=CAB(num_feat=dim, compress_ratio=3, squeeze_factor=30)
        ...
        # self.fusion=nn.Conv2d(4*dim,dim,1,1)
        self.y_aca = None  # 初始化为 None
        self.sim_atd2 = None  # 初始化为 None
    def forward(self, x, td,td2, x_size, params):
        h, w = x_size
        b, n, c = x.shape
        c3 = 3 * c
        ...
        xca = self.ca(xca.permute(0, 3, 1, 2)).reshape(b, n, c)
        ...
        # ATD_CA2: 仅在 y_aca 不为空时执行
        if self.y_aca is not None:
            x_atd2, self.sim_atd2 = self.attn_atd2(self.y_aca, td2, x_size)

        else:
            x_atd2, self.sim_atd2 = None, None

        # AC_MSA: 如果 sim_atd2 为空，传递 sim_atd
        if self.sim_atd2 is not None:
            x_aca, self.y_aca = self.attn_aca(qkv, sim_atd, self.sim_atd2, x_size)

        else:
            x_aca, self.y_aca = self.attn_aca(qkv, sim_atd, None, x_size)
        # SW-MSA
        ...
        if not self.is_last:
            ...
            if self.y_aca is not None and self.sim_atd2 is not None:
                mask_soft2 = self.softmax(self.norm4(self.sim_atd2.transpose(-1, -2)))
                mask_y = self.y_aca.reshape(b, N, c)
                td2 = s2 * td2 + (1 - s2) * torch.einsum('btn,bnc->btc', mask_soft2, mask_y)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.td2 = nn.Parameter(torch.randn([16, dim]), requires_grad=True)
...
class ATD_CA2(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_tokens=64, reducted_dim=10, qkv_bias=True):
        ...
        self.scale = nn.Parameter(torch.ones([16]) * 0.5, requires_grad=True)
        self.softmax = nn.Softmax(dim=-1)
